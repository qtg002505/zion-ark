...
export interface FeedbackEntry {
  ...
  /**
   * 보강 진행 상태 — **출석 격자의 결석 칸에서 남긴 보강 기록에만 있다** (2026-08-15 리드 지시).
   * ⚠️ **출결 원본을 고치는 값이 아니다**(불변식 3). 원본 시트의 결석은 그대로 있고,
   * 「언제 보강하기로 했는지 / 마쳤는지」를 담당자가 옆에 적어 두는 것이다.
   * 격자는 이 값이 있는 칸에 표시를 덧대고, 수강생 상세의 「보강·상담 메모」가 같은 기록을 읽는다.
   */
  makeupState?: "planned" | "done";
  /** 어느 회차의 결석에 대한 보강인지 — 격자 칸과 기록을 잇는 열쇠 */
  sessionNo?: number;
}
