...
/**
 * 출석률 두 가지 — 누적 출석률과 **전추율**.
 *
 * 「전추율」은 2026-08-10 리드가 준 어휘다: **보강을 포함한 실제 출석률.**
 ...
 *
 * ⚠️ 계산 범위는 **최근 8주**(`recentWeeks`)다. 누적 전추율을 내려면 보강 이행 횟수가
...
/** 한 사람의 최근 출결을 세어 전추율과 대면 출석률을 함께 낸다 */
export function rateOf(student: Student): RateBreakdown {
  ...
  return {
    jeonchu: pct(presentCount + makeupDoneCount),
...
export function cohortRates(students: Student[]): { jeonchu: number; presentOnly: number } {
  if (students.length === 0) return { jeonchu: 0, presentOnly: 0 };
  const rows = students.map(rateOf);
  const avg = (pick: (r: RateBreakdown) => number) =>
    Math.round(rows.reduce((a, r) => a + pick(r), 0) / rows.length);
  return { jeonchu: avg((r) => r.jeonchu), presentOnly: avg((r) => r.presentOnly) };
}
