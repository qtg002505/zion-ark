/* 과정 표기 정본 — 폴더 표시 이름이 쓴다. level-labels는 아무것도 안 물어 순환이 없다 */
 ...
 */
/*
  ⛔ **과정 이름은 주제 이름으로 나간다** (2026-08-21 리드 확정 — 학원법.
  색 이름(연두…)으로 냈던 것을 같은 날 「천국 비유 풀이 / 성경 예언과 성취 / 계시록」으로 정정했다).
  저장값(「교분기 초등」·「우수 판서 초등」·「초등 초반 밭갈이」)은 **그대로 두고**
  화면 표기만 바꾼다 — 값을 건드리면 그 폴더의 자료가 빠진다(불변식 10).
  표기 정본은 `content/level-labels.ts`의 `LEVEL_NAME`이고, 여기는 붙임 규칙만 둔다.
  ⚠️ 주제 이름이 길어 뒤에 괄호로 붙인다 — 사이드바(색 묶음 안)는 괄호째 떼어 낸다.
*/
export const FOLDER_DISPLAY_LABELS: Record<string, string> = {
  "교분기 초등": `흐름교육 (${LEVEL_NAME["초등"]})`,
  "교분기 중등": `흐름교육 (${LEVEL_NAME["중등"]})`,
  "교분기 고등": `흐름교육 (${LEVEL_NAME["고등"]})`,
  "우수 판서 초등": `우수 판서 (${LEVEL_NAME["초등"]})`,
  "우수 판서 중등": `우수 판서 (${LEVEL_NAME["중등"]})`,
  "우수 판서 고등": `우수 판서 (${LEVEL_NAME["고등"]})`,
  "초등 초반 밭갈이": `초반 밭갈이 (${LEVEL_NAME["초등"]})`,
};
 ...
 *
 * 병리적 우울·행함부담·이성교제자 같은 **현장 관찰 묶음**에 담당자가 이름을 놓는다.
 * 한 사람이 여러 묶음에 동시에 들어갈 수 있어 (수강생, 묶음) 한 쌍이 한 건이다.
 *
 * ⚠️ **묶음 이름은 코드 값(enum)으로 굳히지 않는다** — 리드 스프레드시트의 표현을
 * 원문 그대로 문자열로 담는다. 표현이 다듬어지면 후방 마이그레이션이 되기 때문이다
 * (상담 테마를 번호로 둔 것과 같은 취급).
 * ⚠️ **사람이 직접 놓는 기록이다** — AI가 판정해 넣지 않는다(불변식 4). 민감한 관찰이라
 * 담당 범위 안에서만 보이고 집계 밖으로 내보내지 않는다(불변식 2).
 * 실연동 시 D1 `student_state_marks` 테이블에 대응한다.
 */
export interface StudentStateMark {
  id: string;
  cohortKey: string;
  /** 묶음 이름 — 리드 스프레드시트 원문 그대로 (예: 「병리적 우울 (기질)」) */
  label: string;
  studentKey: string;
  createdBy: string;
  createdByRole: RoleCode;
