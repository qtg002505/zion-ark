...
- 단계 색(**초 초록** · 중 주황 · 고 남색)은 `LEVEL_TONE`으로만 쓴다. 색값은 `index.css`에 있다
  (초등은 2026-08-18에 하늘색에서 초록으로 바뀌었다)
- **좁은 칸에는 요약 표기를 쓴다** (2026-08-18 리드 전달 — `ELEMENTARY_SHORT_TITLES` ·
  ...
  - ⚠️ 요약은 **차례(인덱스)로** 정본과 이어진다. 한 줄만 밀려도 남의 줄임말이 붙는데
    ...
    **목록을 고치면 테스트도 함께 본다**
- ⛔ **연속 번호 표기를 만들지 않는다 — 학원법 때문이다** (2026-08-15 리드 지시).
  「초등1·2·3」·「초1강」·「중12강」처럼 단계+연속번호는 **학년·학기 편성처럼 읽힌다.**
  화면에는 **단계 + 과수 제목**만 낸다. `lessonNo`는 데이터로 남기고 표기에서만 뺀다.
  - **회차(`N회차`)는 그대로 쓴다** — 출결을 세는 단위다. 다만 **회차를 「N강」으로 적지 않는다**
    (주간계획이 입력은 「회차」로 받고 표기는 「N강」으로 내던 것을 2026-08-15에 고쳤다)
  - 새 화면에 진도를 낼 때는 `shortLessonLabel()`·`sessionKeywordLabel()`을 쓴다 —
    번호를 뺀 표기가 그 두 함수 안에 있어 화면마다 다시 판단할 일이 없다
