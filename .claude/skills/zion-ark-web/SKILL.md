...
- ⛔ **학원법 때문에 두 가지를 화면에서 뺐다.** 연속 번호(2026-08-15)와 **단계 이름**(2026-08-21).
  ...
  - ⚠️ **색 이름(「연두」)은 어디에도 글자로 넣지 않는다.** 2026-08-21에 두 번 고쳐 배웠다:
    좁은 칸에 색 이름을 넣었다가 「연두색 배경 안에 그릇~지팡이 이런 식으로 단어가 들어가는
    거지 색깔 이름이 아니다」, 고르는 자리에 남겼다가 「고르는 자리에 연두 주황 파랑도 빼줘 —
    연두 대신 천국 비유 풀이 / 주황 대신 성경 예언과 성취 / 파랑 대신 계시록으로 정정」.
    **과정을 이름으로 부를 때는 주제 이름**이고 색은 곁에서 거든다.
...
⚠️ **`resize_window`로 모바일로 바꾼 직후에는 반드시 새로고침하고, `innerWidth ===
clientWidth`부터 확인한다** (2026-08-22에 밟았다). 리사이즈만 하고 재면 `innerWidth 587 ·
clientWidth 375`처럼 어긋난 채 레이아웃이 옛 폭으로 남아 **전 라우트가 가로 넘침
145~212px로 오탐**된다 — 진짜 넘침으로 착각하기 딱 좋다. 새로고침 후에는 전부 0이었다.
